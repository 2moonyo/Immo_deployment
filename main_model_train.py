...
class Preprocessing:
    def __init__(self, income_path, zipcode_path, property_path,geodata):
...
class FeatureEngineering:
    ...
    @staticmethod
    def unify_locality_column(data, dfgeo, locality_column="locality", threshold=80):
    # Define the reference lists (French and Dutch spellings)
        french_reference_list = dfgeo["Municipality name (lower case, French)"]
        dutch_reference_list = dfgeo["Municipality name (lower case, Dutch)"]

        # Combine both lists into one reference list and ensure unique entries
        reference_list = pd.concat([french_reference_list, dutch_reference_list]).unique()

        # Function to remove parentheses
        def remove_parentheses(text):
            return re.sub(r'\(.*\)', '', text).strip()

        # Function to match and unify locality names based on a threshold
        def unify_locality_name(locality, reference_list, threshold=80):
            # Get the best match from the combined reference list using RapidFuzz
            result = process.extractOne(locality, reference_list, scorer=fuzz.ratio)
            
            # Unpack the result to get the best match and score
            best_match, score, _ = result
            
            # If the similarity score is above the threshold, return the best match, else return the original name
            if score >= threshold:
                return remove_parentheses(best_match)
            else:
                return remove_parentheses(locality)  # If no match above threshold, return the original name

        # Apply the function to unify locality names in the DataFrame and update the unified_locality column
        data['unified_locality'] = data[locality_column].apply(lambda x: unify_locality_name(x, reference_list, threshold))

        #Export the best matches to a CSV
        data[['locality', 'unified_locality']].to_csv('unified_locality_matches.csv', index=False)
        
        data["locality"] = data["unified_locality"]
        return data
...
def main():
    ...
    income_path = "/Users/irisvirus/Desktop/Becode/Python/Projects/Regression/regression/Delivery/utils/INCOME DATA 2022.csv"
    zipcode_path = "/Users/irisvirus/Desktop/Becode/Python/Projects/Regression/regression/Delivery/utils/zipcodes_num_nl_new_Tumi.xls"
    property_path = "/Users/irisvirus/Desktop/Becode/Python/Projects/Regression/regression/Delivery/utils/properties_data_cleaned_05_12_14H30.csv"
    geodata = "/Users/irisvirus/Desktop/Becode/Python/Projects/Deployment/Immo_deployment/Belgium Geospatial data/OPEN DATA/georef-belgium-municipality.csv"
    # Preprocessing
    ...
    property_data = feature_engineering.unify_locality_column(property_data, geodata)
